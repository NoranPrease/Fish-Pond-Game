...
/*Questions
 *@turnFish() --  Is 'continue' good code for doing nothing if the fish is
 *surrounded by rocks?
 *
 *@moveFish() -- do i include a turnFish() in there?(It says something about it
 *in the comments
 */
...
public class Model {
	...
	public boolean isSpaceAvailable(int r, int c) {
		//Iterates through current fish array, sees if any have same coordinates
		//as the parameter
		...
		//Iterates through current plant array, sees if any have same coordinates
		//as the parameter
	...
	/** Copy Constructor.
	 * <p>
	 * Since landscape is immutable, it is to be copied with just a shallow copy.
	 * Fish and Plants are mutable, so they must be copied with a DEEP copy!
	 * (WARNING:  Each fish and each plant must be copied.)
	 */
	public Model(Model other) {
		boolean[][] newLandscape = new boolean[other.getRows()][other.getCols()];
		for(int row=0; row<other.getRows(); row++){
			for(int col=0; col<other.getCols(); col++){
				newLandscape[row][col] = other.landscape[row][col];
			}
		}
		landscape = newLandscape;
		
		//Creates deep copy of current object fish array
		ArrayList<Fish> newFishArray = new ArrayList<Fish>();
		for(Fish fish : other.fish){
			newFishArray.add(new Fish(fish));
		}
		fish = newFishArray;
		
		//Creates deep copy of current object plant array
		ArrayList<Plant> newPlantArray = new ArrayList<Plant>();
		for(Plant plant : other.plants){
			newPlantArray.add(new Plant(plant));
		}
	...
	public static void fishEatPlant(Fish f, Plant p) {
		...
		else if(p.getSize() > Plant.PLANT_BITE_SIZE){
	...
	public void removeDeadFish() {
		ArrayList<Fish> newFish = new ArrayList<Fish>(fish);
		for(Fish fish : this.fish){
	...
	private boolean fishIsSurroundedByRocks(Fish f) {
		if(getShape((f.getRow())-1,f.getCol())==ROCK
				&& getShape(f.getRow(),(f.getCol())-1)==ROCK
				&& getShape((f.getRow())+1,f.getCol())==ROCK
				&& getShape(f.getRow(),(f.getCol())+1)==ROCK){
			return true;
		}else return false;
	}
	...
	public void turnFish() {
		for(Fish fish : this.fish){
			if(fish.isAlive()){
				if(fishIsSurroundedByRocks(fish)==true){
					continue;
				}
				if(fish.getDirection() != Fish.UP
						&& fish.getDirection() != Fish.DOWN
						&& fish.getDirection() != Fish.LEFT
						&& fish.getDirection() != Fish.RIGHT){
					throw new IllegalFishDirectionException(fish.getDirection());
				}
				//Loops through until the method nextItemIsRock returns false
				while(nextItemIsRock(fish)==true){
	...
	public void moveFish() {
		for(Fish fishCount : this.fish){
			if(fishCount.isAlive()==true){
				if(fishIsSurroundedByRocks(fishCount)==true){
					continue;
				}
				fishCount.move();
				for(Plant plantCount : this.plants){
					if(fishCount.getRow()==plantCount.getRow()
							&& fishCount.getCol()==plantCount.getCol()
							&&plantCount.isAlive()){
						fishEatPlant(fishCount,plantCount);
					}
				}
				for(Fish otherFish : this.fish){
					if(fishCount.getCol()==otherFish.getCol()
							&&fishCount.getRow()==otherFish.getRow()
							&&otherFish.isAlive()==true && fishCount!=otherFish){
						fishCount.fight(otherFish);
					}
				}
			}
		}
	...
	public void addFish(Fish f){
		if(getShape(f.getRow(),f.getCol())==ROCK){
			throw new IllegalFishPositionException
								(IllegalFishPositionException.FISH_OVER_ROCK);
		}
		else if(isAnotherFishHere(f.getRow(),f.getCol())==true){
			throw new IllegalFishPositionException
	...
	/**
	 * Checks to see if the fish's next move will land him on top of a rock
	 * @param fishIn : The fish to be checked
	 * @return true if rock is one move in front of the fish
	 */
	private boolean nextItemIsRock(Fish fishIn){
		int direction = fishIn.getDirection();
		if(direction==Fish.UP){
			if(getShape(fishIn.getRow()-1,fishIn.getCol())==ROCK){
				return true;
			}
		}
		if(direction==Fish.DOWN){
			if(getShape(fishIn.getRow()+1,fishIn.getCol())==ROCK){
				return true;
			}
		}
		if(direction==Fish.LEFT){
			if(getShape(fishIn.getRow(),fishIn.getCol()-1)==ROCK){
				return true;
			}
		}
		if(direction==Fish.RIGHT){
			if(getShape(fishIn.getRow(),fishIn.getCol()+1)==ROCK){
				return true;
			}
		}
	...
	/**
	 * Checks if a fish already exists in this array
	 * @param r : row
	 * @param c : column
	 * @return boolean of whether a fish already exists in the same row/col
	 */
	private boolean isAnotherFishHere(int r, int c){
		for(Fish fish : this.fish){
			if(fish.getCol()==c && fish.getRow()==r
					&& fish.isAlive()){
				return true;
